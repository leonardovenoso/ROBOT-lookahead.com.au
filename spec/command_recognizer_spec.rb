...
RSpec.describe CommandRecognizer do 
	...
	it 'splits the full valid command string in a hash' do
		hash_with_command_and_parameters = @commnad_recognizer.get_hash_with_command_and_parameters(@full_valid_command)
	...
	it 'recieves an unknown command hash when coordinates are invalid' do
		hash_with_command_and_parameters = @commnad_recognizer.get_hash_with_command_and_parameters(@full_invalid_coordinate_command)
	...
	it 'recieves an unknown command hash when orientation is invalid' do
		hash_with_command_and_parameters = @commnad_recognizer.get_hash_with_command_and_parameters(@full_invalid_orientation_command)
		expect(hash_with_command_and_parameters[:command]).to eq 'unknown'
	end

	it 'splits the move command string in a hash' do
		hash_with_command_and_parameters = @commnad_recognizer.get_hash_with_command_and_parameters("move")
	...
	it 'validates orientation with a valid parameter' do
		expect(@commnad_recognizer.send(:is_orientation_valid?, "NORTH")).to eq true		
		expect(@commnad_recognizer.send(:is_orientation_valid?, "SOUTH EAST")).to eq true		
		expect(@commnad_recognizer.send(:is_orientation_valid?, "WEST")).to eq true		
	end	

		it 'validates orientation with an invalid parameter' do
		expect(@commnad_recognizer.send(:is_orientation_valid?, "NOORTH")).to eq false		
		expect(@commnad_recognizer.send(:is_orientation_valid?, "zOUTH EAST")).to eq false		
		expect(@commnad_recognizer.send(:is_orientation_valid?, "westtt")).to eq false		
	end	
